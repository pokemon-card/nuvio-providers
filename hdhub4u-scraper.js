...
function fetchAndUpdateDomain() {
    return axios.get(DOMAINS_URL, { httpsAgent: agent })
        .then(response => {
...
function getRedirectLinks(url) {
    return axios.get(url, { headers: HEADERS, httpsAgent: agent })
        .then(response => {
            ...
            if (wpHttp && data) {
                return axios.get(`${wpHttp}?re=${data}`, { headers: HEADERS, httpsAgent: agent })
                    .then(directLinkResponse => {
...
function pixelDrainExtractor(link) {
    return Promise.resolve().then(() => {
        ...
        return axios.get(infoUrl, { httpsAgent: agent })
            .then(({ data: info }) => {
                if (info && info.name) {
...
function streamTapeExtractor(link) {
    ...
    return axios.get(normalizedLink, { headers: HEADERS, httpsAgent: agent })
        .then(res => {
...
function hubStreamExtractor(url, referer) {
    ...
    return axios.get(url, { headers: { ...HEADERS, Referer: referer }, httpsAgent: agent })
        .then(response => {
...
function hubCloudExtractor(url, referer) {
    ...
    return axios.get(currentUrl, { headers: { ...HEADERS, Referer: referer }, httpsAgent: agent })
        .then(pageResponse => {
            ...
            if (!currentUrl.includes("hubcloud.php")) {
                ...
                if (scriptUrlMatch && scriptUrlMatch[1]) {
                    ...
                    return axios.get(finalUrl, { headers: { ...HEADERS, Referer: currentUrl }, httpsAgent: agent })
                        .then(secondResponse => ({ pageResponse: secondResponse, finalUrl }));
                }
            }
            return { pageResponse, finalUrl };
        })
        .then(({ pageResponse, finalUrl }) => {
            ...
            const processElements = elements.map(element => {
                ...
                if (text.includes("Download File")) {
                    ...
                    return axios.get(`${link}/download`, {
                    ...
                    const processRedirects = (i) => {
                        ...
                        return axios.get(currentRedirectUrl, { maxRedirects: 0, validateStatus: null, httpsAgent: agent })
                            .then(response => {
...
function search(query) {
    return getCurrentDomain()
        .then(currentDomain => {
            ...
            return axios.get(searchUrl, { headers: HEADERS, httpsAgent: agent });
        })
        .then(response => {
            const $ = cheerio.load(response.data);
...
function getCinemetaData(imdbId, tvType) {
    ...
    return axios.get(cinemetaUrl, { httpsAgent: agent })
...
function getDownloadLinks(mediaUrl) {
    return getCurrentDomain()
        .then(currentDomain => {
            ...
            return axios.get(mediaUrl, { headers: HEADERS, httpsAgent: agent });
        })
        .then(response => {
            const $ = cheerio.load(response.data);
                        ...
                        if (isDirectLinkBlock) {
                            ...
                            return Promise.all(redirectLinks.map(redirectLink =>
                                getRedirectLinks(redirectLink)
                                    .then(resolvedUrl =>
                                        axios.get(resolvedUrl, { headers: HEADERS, httpsAgent: agent })
                                            .then(episodeDocPage => {
...
function getNumericInput(question, min = 1, max = null) {
    const validateInput = () => {
        return getUserInput(question)
            .then(input => {
                ...
                if (isNaN(num) || num < min || (max && num > max)) {
                    ...
                    return validateInput(); // Recursively call for another attempt
                }
...
function main() {
    ...
    return fetchAndUpdateDomain()
        .then(() => {
            // 1. GET USER INPUT FOR MEDIA TYPE AND DETAILS
            return getUserInput("Enter the name of the movie or TV show: ");
        })
        .then(showName => {
            if (!showName) {
            ...
            return getUserInput("Is this a TV show or movie? (tv/movie): ")
                .then(mediaType => {
                    ...
                    if (isTvShow) {
                        ...
                        return getNumericInput("Enter season number: ", 1)
                            .then(season => {
                                targetSeason = season;
                                return getUserInput("Do you want a specific episode or all episodes? (specific/all): ");
                            })
                            .then(wantSpecificEpisode => {
                                if (wantSpecificEpisode.toLowerCase().startsWith('s') || wantSpecificEpisode.toLowerCase().startsWith('specific') || !isNaN(parseInt(wantSpecificEpisode))) {
                                    ...
                                    if (!isNaN(directEpisodeNum) && directEpisodeNum > 0) {
                                        ...
                                        return getNumericInput("Enter episode number: ", 1)
                                            .then(episode => {
                            ...
                            .then(() => {
                                ...
                                return search(searchQuery).then(searchResults => ({
                                    searchResults,
                                    isTvShow,
                                    targetSeason,
                                    targetEpisode,
                ...
                .then(({ searchResults, isTvShow, targetSeason, targetEpisode, showName, searchQuery }) => {
                    ...
                    return getDownloadLinks(mediaToProcess.url)
                        .then(({ finalLinks, isMovie }) => {
